...
/* 
* Get User By Id Function - Gets user from DB by Id
*
* @param {Int} id 
* @param {function} callback - The callback function.
...
module.exports.getUserById = function(id, callback){
	q = 'SELECT id, "restaurantName", email, password FROM "Restaurant" WHERE id = $1;';
	values = [id];
	query(q, values, function(err, result){
		if(err) {
			return callback(err);
...
/* 
* Get User By Email Function - Gets user from DB by email
*
* @param {Int} Email
* @param {function} callback - The callback function.
* @return {function} Returns callback function with error or user
...
module.exports.getUserByEmail = function(email, callback){
	q = 'SELECT id, "restaurantName", email, password FROM "Restaurant" WHERE email = $1;';
	values = [email];
	query(q, values, function(err, result){
		if(err) {
			return callback(err);
