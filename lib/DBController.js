...
module.exports.createOffer = function(newOffer, callback){
	...
	// Query string
	var q = 'INSERT INTO "Offers" ("restName", name, type, price, description, "startDate", "endDate", photo, "restId") VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id';
...
// Get offers by typeQuery object, priceQuery object and searchBarQuery object.
module.exports.getOffers = function(typeQuery, priceQuery, searchBarQuery, cb){

	var results = [];
	// Use promises to run all queries and only when all queries have results return
	// the results
	when.all([
		query2(typeQuery.queryString, typeQuery.values),
		query2(priceQuery.queryString, priceQuery.values),
		query2(searchBarQuery.queryString, searchBarQuery.values)
		]).spread(function(typeResults, priceResults, searchBarResults){
			results = [typeResults[0], priceResults[0], searchBarResults[0]];
			cb(null, results);
		});
...
module.exports.findAllUserInfo = function(email,cb) {
  var values = [email];
  var q = 'SELECT "id", "restaurantName", "image", "description", "phonenumber", "website"  FROM "Restaurant" WHERE email = $1';
...
module.exports.getRestaurantById = function(id, cb){
	var values = [id];
	var q = 'SELECT id "restaurantName", "image", "description", "phonenumber", "website" FROM "Restaurant" WHERE id = $1';
